  ...
  def perform 
    ...
    count_batch.times do
      ...
      create_values = values.map do |val| 
        statistic = { updated_at: val[-1] }
        statistic[:views]    = val[2]
        statistic[:comments] = val[3] if val[3].present?
        ...
        "(#{[ActiveRecord::Base.connection.quote(val[0]),
         ...
         "#{ActiveRecord::Base.connection.quote(val[4].to_json)}",
         "#{ActiveRecord::Base.connection.quote([statistic].to_json)}",
      ...
      MainDbRecord.connection.execute("INSERT INTO posts AS p
        (link, tg_id, views, comments, links, statistic, has_photo, has_video, published_at, next_post_at, html, is_repost, channel_id, skip_screen, feed_hours, top_hours, last_parsed_at, created_at, updated_at)
