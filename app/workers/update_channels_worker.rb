...
class UpdateChannelsWorker
  ...
  def perform 
    ...
    count_batch   = (data_count / UPDATE_COUNT.to_f).ceil
    ...
    count_batch.times do
      values = Redis0.lrange('channels_data', 0, (UPDATE_COUNT - 1))
      ...
      values.each do |v| 
        ...
        update_values << "(#{columns.map.with_index do |col, inx|
          ...
          [:update_info_at, :last_post_date].include?(col) ? ar_conn.quote(val[inx]) : ar_conn.quote_default_expression(val[inx], find_column)
        end.join(', ')})"
      ...
      Redis0.ltrim('channels_data', UPDATE_COUNT, -1)
    end
  end
  ...
  def update_channels(data)
    MainDbRecord.connection.execute("UPDATE channels AS c SET 
      ...
      update_info_at = CAST(d.update_info_at AS TIMESTAMP WITHOUT TIME ZONE),
      by_telethon_parse = d.by_telethon_parse,
      last_post_date = CASE WHEN d.last_post_date IS NOT NULL THEN CAST(d.last_post_date AS TIMESTAMP WITHOUT TIME ZONE) ELSE c.last_post_date END
