  ...
  def perform 
    ...
    count_batch.times do
      ...
      values.each do |v| 
        ...
        if channel_data_changed?(last_channel_data, subscribers, title, description, parse_mode)
          update_values << "(#{[ActiveRecord::Base.connection.quote(channel_id),
             ...
             ActiveRecord::Base.connection.quote((parse_mode == 'by_web_parse')),
             ActiveRecord::Base.connection.quote((parse_mode == 'by_telethon_parse')),
             ActiveRecord::Base.connection.quote(val[7]),
             ActiveRecord::Base.connection.quote(val[8])
           ].join(', ')})"

           Redis0.set("last_channel_data:#{channel_id}", [subscribers, title, description, parse_mode].to_json)
         end
